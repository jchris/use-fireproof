...
While encryption is a vital aspect of data security, there may be instances, particularly during testing or development, or for content-delivery workloads, where you might want to disable it. For such cases, Fireproof allows disabling encryption via the configuration option `{ public: true }`. Simply set this to disable encryption. This functionality also allows you to create unencrypted database files for publishing.
